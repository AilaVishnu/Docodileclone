...
data class BillTemplateDTO(
    ...
    // Raw JSON string — caller decodes and re-encodes. Matches how the entity
    // stores it; saves us from defining the whole template shape server-side
    // just to pass it through.
    val config: String
)

data class BillTemplateRequest(
    val name: String,
...
@RestController
@RequestMapping("/api/tenant/bill-templates")
class BillTemplateController(
    private val repo: BillTemplateRepository,
) {
    private fun toDto(t: BillTemplate) = BillTemplateDTO(
        id = t.id,
        name = t.name,
    ...
    fun list(): List<BillTemplateDTO> =
        repo.findAllByOrderByCreatedAtAsc().map(::toDto)

    @PostMapping
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    @Transactional
    fun create(@RequestBody request: BillTemplateRequest): ResponseEntity<BillTemplateDTO> {
        ...
        val now = Instant.now()
        val saved = repo.save(BillTemplate(
    ...
    fun update(
        ...
        @RequestBody request: BillTemplateRequest
    ): ResponseEntity<BillTemplateDTO> {
    ...
    fun delete(@PathVariable id: UUID): ResponseEntity<Void> {
        ...
        // Promote the next-oldest template to default so the clinic always has a
        // default bill template available.
        if (wasDefault) {
