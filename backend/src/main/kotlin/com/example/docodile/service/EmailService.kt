...
class EmailService(private val mailSender: JavaMailSender) {
    ...
    @Async
    fun sendWelcomeEmail(to: String, name: String, setupLink: String) {
        try {
            val msg: MimeMessage = mailSender.createMimeMessage()
            val helper = MimeMessageHelper(msg, false, "UTF-8")
            helper.setTo(to)
            helper.setSubject("Welcome to Docodile — Set up your password")
            helper.setText(buildWelcomeHtml(name, setupLink), true)
            mailSender.send(msg)
    ...
    private fun buildWelcomeHtml(name: String, setupLink: String): String = """
        <!DOCTYPE html>
        <html>
