...
// "owe" a deposit. Schema-scoped (no clinic_id).
@Service
class PatientDepositService(
    ...
    fun getDeposit(patientId: UUID): PatientDepositDTO {
        val patient = patientRepository.findById(patientId).orElse(null)
            ?: throw IllegalArgumentException("Patient not found")
        return buildDTO(patientId, patient.deposit ?: zero)
    }

    private fun buildDTO(patientId: UUID, net: BigDecimal): PatientDepositDTO {
        val ledger = ledgerRepository.findHistory(patientId).map {
