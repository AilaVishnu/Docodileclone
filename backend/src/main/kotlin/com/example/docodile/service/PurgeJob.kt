...
class PurgeJob(
    ...
    // per clinic schema. (Revocation now lives on user_session.revoked_at.)
    @Scheduled(cron = "0 15 2 * * SUN")
    fun purgeExpiredSessions() {
        perClinic.forEachActiveClinic("purge expired sessions") {
            val deleted = userSessionRepository.deleteExpired(Instant.now())
            if (deleted > 0) log.info("PurgeJob: deleted {} expired user_session rows", deleted)
        }
    }
    ...
    fun reportPurgeablePatients() {
        perClinic.forEachActiveClinic("report purgeable patients") {
            ...
            if (purgeable.isEmpty()) return@forEachActiveClinic
            log.warn(
